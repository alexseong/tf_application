...
trunc_normal = lambda stddev: tf.initializers.TruncatedNormal(0.0, stddev)
...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p):
    ...
    with tf.name_scope(name) as scope:
        kernel = tf.compat.v1.get_variable(scope+"w", shape=[kh, kw, n_in, n_out], dtype=tf.float32, initializer=tf.keras.initializers.GlorotNormal)
        conv = tf.compat.v1.nn.conv2d(input_op, kernel, (1, dh, dw, 1), padding='SAME')
        bias_init_val = tf.constant(0.0, shape=[n_out], dtype=tf.float32)
        biases = tf.Variable(bias_init_val, trainable=True, name='b')
        activation = tf.nn.relu(tf.nn.bias_add(conv, biases), name=scope)
        p += [kernel, biases]
        print("out: ", activation.get_shape())
...
def fc_op(input_op, name, n_out, p):
    ...
    with tf.name_scope(name) as scope:
        kernel = tf.compat.v1.get_variable(scope+"W", shape=[n_in, n_out], dtype=tf.float32, initializer=tf.initializers.GlorotUniform())
...
def inception_v3_arg_scope(weight_decay=0.00004, stddev=0.1, batch_norm_var_collection='moving_var'):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected], weights_regularizer=slim.l2_regularizer(weight_decay)):
        with slim.arg_scope(
            [slim.conv2d],
            weights_regularizer = tf.initializers.TruncatedNormal(stddev=stddev),
    ...
    with tf.compat.v1.variable_scope(scope, 'InceptionV3', [inputs]):
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
            ...
            with tf.compat.v1.variable_scope('Mixed_6a'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    branch_3 = slim.max_pool2d(net, [3, 3], stride=2, padding='VALID', scope='MaxPool_3a_3x3')
            ...
            with tf.compat.v1.variable_scope('Mixed_6b'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    ...
                    branch_2 = slim.conv2d(branch_1, 128, [7, 1], scope='Conv2d_2b_7x1')
                    branch_2 = slim.conv2d(branch_1, 128, [1, 7], scope='Conv2d_2c_1x7')
                    branch_2 = slim.conv2d(branch_1, 128, [7, 1], scope='Conv2d_2d_7x1')
                    branch_2 = slim.conv2d(branch_1, 192, [1, 7], scope='Conv2d_2e_1x7')
                with tf.compat.v1.variable_scope('Branch_3'):
                    branch_3 = slim.avg_pool2d(net, [3, 3], scope='AvgPool_3a_3x3')
            ...
            with tf.compat.v1.variable_scope('Mixed_6c'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    ...
                    branch_2 = slim.conv2d(branch_1, 160, [7, 1], scope='Conv2d_2b_7x1')
                    branch_2 = slim.conv2d(branch_1, 160, [1, 7], scope='Conv2d_2c_1x7')
                    branch_2 = slim.conv2d(branch_1, 160, [7, 1], scope='Conv2d_2d_7x1')
                    branch_2 = slim.conv2d(branch_1, 192, [1, 7], scope='Conv2d_2e_1x7')
                with tf.compat.v1.variable_scope('Branch_3'):
                    branch_3 = slim.avg_pool2d(net, [3, 3], scope='AvgPool_3a_3x3')
                    branch_3 = slim.conv2d(branch_3, 192, [1, 1], scope='Conv2d_3b_1x1')
            ...
            with tf.compat.v1.variable_scope('Mixed_6e'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    branch_2 = slim.conv2d(net, 160, [1, 1], scope='Conv2d_2a_1x1')
                    branch_2 = slim.conv2d(branch_1, 192, [7, 1], scope='Conv2d_2b_7x1')
                    branch_2 = slim.conv2d(branch_1, 192, [1, 7], scope='Conv2d_2c_1x7')
                    branch_2 = slim.conv2d(branch_1, 192, [7, 1], scope='Conv2d_2d_7x1')
                    branch_2 = slim.conv2d(branch_1, 192, [1, 7], scope='Conv2d_2e_1x7')
                with tf.compat.v1.variable_scope('Branch_3'):
                    branch_3 = slim.avg_pool2d(net, [3, 3], scope='AvgPool_3a_3x3')
                    branch_3 = slim.conv2d(branch_3, 192, [1, 1], scope='Conv2d_3b_1x1')
            ...
            with tf.compat.v1.variable_scope('Mixed_7a'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    branch_2 = slim.max_pool2d(net, [3, 3], scope='MaxPool_2a_3x3')                
            ...
            with tf.compat.v1.variable_scope('Mixed_7b'):
                ...
                with tf.compat.v1.variable_scope('Branch_2'):
                    ...
                    branch_2 = slim.conv2d(branch_2, 384, [3, 3], scope='Conv2d_2b_1x1')
                    branch_2 = tf.concat([
    ...
    with tf.compat.v1.variable_scope(scope, 'InceptionV3', [inputs, num_classes], reuse=reuse) as scope:
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
            ...
            with tf.compat.v1.variable_scope('logits'):
                ...
                net = slim.dropout(net, keep_prob=drop_out_keep_prob, scope='Dropout_1b')
...
#    inputs = tf.compat.v1.random_uniform((batch_size, height, width, 3))
    inputs = tf.Variable(tf.random.normal([batch_size, height, width, 3]))
#    inputs = tf.Variable(tf.random.normal())
